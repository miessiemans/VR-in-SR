Shader "Custom/Relativistic Shader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {   // map 3D world dir vector to equirect UV
                dir = normalize(dir);
            ...
            float3 doppler_colour(float3 colour, float D, float cosTheta, float beta)
            {
                ...
                float colour_multiplier = max(beta, 0.0) / 0.7 * 2.5;

                if (D < 1.0)
                {   // push rgb values more blue or red depending on D
                    float shift = clamp((1.0 - D) * colour_multiplier, 0.0, 1.0);
                    colour.b = lerp(colour.b, 1.0, shift);
                    colour.r *= lerp(1.0, 0.2, shift);
                ...
                if (beta > 0.5) // show spectrum lines at edges of the tunnel
                {   // numbers are not physically significant, but after playing around
                    // these offered the spectrums without overly beaming the centre
                    ...
                    float spectrum_strength = ((beta - 0.5) / 0.5);
                    spectrum_strength = spectrum_strength * spectrum_strength * 1;
